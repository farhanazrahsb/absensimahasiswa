...
<div class="title">
	...
	<i class="fa fa-home"></i> <a href="admin.php">Dashboard</a> / <i class="fa fa-folder-open-o"></i> Pilih Kelas
	</div>
	...
	<?php
		...
		$kelas=mysqli_query($koneksi, "select * from kelas where kode_jurusan='$kode_jurusan' order by kelas asc");
		$no=1;
		//untuk mencari jumlah
		echo "Jumlah Kelas : ".$jumlah_kelas=mysqli_num_rows($kelas);
		echo " >> Jumlah Siswa : ".$jumlah_siswa=mysqli_num_rows(mysqli_query($koneksi, "select * from input_mhs where kode_jurusan='$kode_jurusan'"));
		echo "<br><br>";
		
		while($row=mysqli_fetch_array($kelas)){
			//mencari jumlah siswa di masing-masing kelas
			$siswa=mysqli_query($koneksi, "select * from input_mhs where kode_kelas='$row[kode_kelas]'");
			$jumlah=mysqli_num_rows($siswa);
			?>
			<tr>
				<td><?php echo $no ?></td>
				<td align="center"><a href="detail_kelas.php?&kode_kelas=<?php echo $row['kode_kelas']; ?>" type="button" class="btn btn-info btn-md" title="Detail Kelas"><?php echo $row['kelas']; ?></a></td>
				<td align="center">Ada<b> <?php echo $jumlah; ?></b> Siswa</td>
			</tr>
	<?php
	$no++;
	}
